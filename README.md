...
异常分支：
  - 禁收物处置：司机装车时发现油漆桶/电池/胶水等禁收物 → 拍照上报暂停装车 → 物业确认处理方式（退回/加价/专项处理）→ 业主收到通知（禁收原因+处理方式+费用影响）
  - 进场受阻：司机到场后因电梯维修/车库限高/道路管制/业主未在家无法进场 → 记录到场时间和现场照片 → 平台重新安排车辆时段 → 清运公司结算空跑费 → 物业追溯放行责任
```
